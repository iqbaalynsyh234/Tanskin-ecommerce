...
<style type="text/css">
.gbrupload {
    background-image: url('<?php echo base_url() ?>assets/image/product/placeholder.jpg');
    width: 100%;
    height: 100%;
    margin-right: 10px;
...
<form class="box" action="<?php echo base_url().'process_entersite/updateitems' ?>" method="post" enctype="multipart/form-data">
...
<div class="box-body">
  <div class="row row-mar">
    ...
    <input type="text" class="form-control" name="itemcode" value="<?php echo $item['ItemCode'] ?>" readonly>
    </div>
    </div>
    ...
    <p>
        ...
        <input type="radio" name="publish" value="11" <?php echo ($item['publish'] == '11') ? 'checked' : ''; ?> checked>
        Yes
        </label>
        &nbsp;&nbsp;&nbsp;&nbsp;
        <label style="margin-bottom: 0px;">
        <input type="radio" name="publish" value="01" <?php echo ($item['publish'] == '01') ? 'checked' : ''; ?>>
        No
        </label>
    </p>
    </div>
    ...
    <input type="text" class="form-control" name="itemdesc" value="<?php echo $item['ItemNmDesc'] ?>">
    </div>
    </div>
    ...
    <?php
    $ItemSubcate = (!empty($item['ItemSubcate'])) ? explode(',', $item['ItemSubcate']) : $item['ItemSubcate'];

    foreach ($all_hirarki->result_array() as $key => $cat) {
        $selected = (!empty($ItemSubcate[$key]) == $cat['ID_cat'])? 'selected' : '';
    ?>
        <option value="<?php echo $cat['ID_cat'] ?>" <?php echo $selected  ?>>
    ...
    <div class="form-group">
    <label>Price</label>
    <input type="number" class="form-control" name="price" placeholder="Rp" value="<?php echo $item['ItemPrice'] ?>" required>
    </div>
    </div>
    <div class="col-sm-4 col-pad">
    <div class="form-group">
    ...
    <div class="input-group input-group disc-panel">
        <div class="input-group-btn">
            ...
            <span id="disc_concept">%</span></button>
        </div>
        <input type="hidden" name="disc_param" value="%" id="disc_param">
        <!-- /btn-group -->
        <input type="number" max="100" min="0" name="disc" class="form-control" value="<?php echo $item['ItemDisc'] ?>">
    </div>
    </div>
    </div>
    <div class="col-sm-4 col-pad">
    <div class="form-group">
    <label>Weight</label>
    <input type="number" class="form-control" name="weight" min="0" placeholder="Gram" value="<?php echo $item['ItemWeight'] ?>" required>
    </div>
    </div>
    ...
    <textarea class="form-control textarea" rows="5" name="description" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;"><?php echo $item['ItemDescription'] ?></textarea>
    </div>
    </div>
    </div>
    </div>
...
if($listcolor->num_rows() > 0){
    $selected = ($item['ItemType'] == '2') ? 'selected' : '';
    $type2 = '<option value="2" '.$selected.'>-- Color --</option>';
}else{
    $type2 = '';
}
if($listsize->num_rows() > 0){
    $selected = ($item['ItemType'] == '3') ? 'selected' : '';
    $type3 = '<option value="3" '.$selected.'>-- Size --</option>';
}else{ 
    $type3 = '';
}
if($listcolor->num_rows() > 0 && $listsize->num_rows() > 0){
    $selected = ($item['ItemType'] == '4') ? 'selected' : '';
    $type4 = '<option value="4" '.$selected.'>-- Size & Color --</option>';
}else{
    $type4 = '';
}
?>
    ...
    <div class="form-group">
        ...
        <select id="product_type" name="product_type" class="form-control">
          <option value="1" <?php echo ($item['ItemType'] == '1') ? 'selected' : ''; ?>>-- Normal --</option>
          <?php echo $type2 ?>
          <?php echo $type3 ?>
          <?php echo $type4 ?>
        </select>
    ...
    <!-- TYPE 1 -->
    <div id="type_1" class="form-group row row-mar <?php echo ($item['ItemType'] != '1') ? 'hide' : ''; ?>">
        <div class="col-sm-3 col-pad">
            <label>Stock</label>
            <input type="number" name="stock[]" value="<?php echo ($item['ItemType'] == '1') ? $item['stock'][0]['stock']: '' ?>" class="form-control type1_rec"  <?php echo ($item['ItemType'] != '1') ? 'disabled' : 'required'; ?>>
        </div>
    </div>
    ...
    <div id="type_2" class="hide">
        ...
        <div class="col-sm-3 col-pad">
            <label>Stock</label>
            <input type="number" name="stock[]" class="form-control type2_rec" required>
        </div>
        </div>
    </div>
    <?php } 
    ...
    ?>
    <!-- TYPE 3 -->
    <div id="type_3" class="<?php echo ($item['ItemType'] != '3') ? 'hide' : ''; ?>">
        <div class="form-group row row-mar">
            <div class="col-sm-12 col-pad text-right product-default">
                <label style="margin-bottom: 0px;">
        ...
        <?php  
        $def_size  = (!empty($item['stock'][0]['size'])) ? $item['stock'][0]['size'] : '';
        $def_stock = (!empty($item['stock'][0]['stock'])) ? $item['stock'][0]['stock'] : '';
        foreach($listsize->result() as $size){
        ?>
        <option value="<?php echo $size->ID_size ?>" <?php echo ($def_size == $size->ID_size) ? 'selected' : ''; ?>><?php echo strtoupper($size->Size) ?></option>
        <?php } ?>
        </select>
        </div>
        <div class="col-xs-3 col-pad">
            <label>Stock</label>
            <input type="number" name="stock[]" class="form-control type3_rec" value="<?php echo $def_stock ?>" required>
        </div>
        ...
        <div class="next_size">
            <?php if(count($item['stock']) > 1) { 
                foreach ($item['stock'] as $key => $value) {
                    if($key > 0){
            ?>
            <div class="form-group  row row-mar add-new-stck"> 
                <div class="col-xs-6 col-pad"> 
                    <select name="size[]" class="form-control select2 disselectSize" style="width: 100%" required> 
                        <option value="">-- Select Size --</option> 
                        <?php foreach($listsize->result() as $size){?> 
                            <option value="<?php echo $size->ID_size ?>" <?php echo ($value['size'] == $size->ID_size) ? 'selected' : ''; ?>><?php echo strtoupper($size->Size) ?></option> 
                        <?php } ?> 
                    </select> 
                </div> 
                <div class="col-xs-3 col-pad"> 
                    <div class="input-group input-group"> 
                        <input type="number" name="stock[]" class="form-control" value="<?php echo $value['stock'] ?>" required> 
                        <span class="input-group-btn"> 
                            <button type="button" class="btn btn-default btn-stck-remove btn-flat"><i class="fa fa-trash"></i></button> 
                        </span> 
                    </div> 
                </div> 
            </div>
    ...
    <div class="wrap-color--image form-group row row-mar">
      <div class="col-sm-4 col-pad image--preview">
        <div class="gbrupload" style="<?php echo 'background-image:url('.base_url('assets/image/product/'.$item['images'][0]['image1']).')' ?>"><img src="<?php echo base_url().'assets/image/product/wrap.png' ?>" class="img-responsive"></div>
      ...
      <div class="col-sm-4 col-pad image--preview">
        <div class="gbrupload" style="<?php echo 'background-image:url('.base_url('assets/image/product/'.$item['images'][0]['image2']).')' ?>"><img src="<?php echo base_url().'assets/image/product/wrap.png' ?>" class="img-responsive"></div>
        <div class="custom-file-input">
            <span></span>
            <span>
            <i class="icon-picture"></i>&nbsp; &nbsp;Browse
            <input type="file" class="btn-img-prev" name="imageseco[]">
            </span>
        </div>
      </div>
      <div class="col-sm-4 col-pad image--preview">
        <div class="gbrupload" style="<?php echo 'background-image:url('.base_url('assets/image/product/'.$item['images'][0]['image3']).')' ?>"><img src="<?php echo base_url().'assets/image/product/wrap.png' ?>" class="img-responsive"></div>
        <div class="custom-file-input">
            <span></span>
            <span>
            <i class="icon-picture"></i>&nbsp; &nbsp;Browse
            <input type="file" class="btn-img-prev" name="imagethir[]">
            </span>
        </div>
      </div>
    </div>
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" name="seo_title" value="<?php echo $item['meta_title'] ?>">
    </div>
    <div class="form-group">
        <label>Meta Descriptions</label>
        <input type="text" class="form-control" name="seo_descriptions" value="<?php echo $item['meta_deskripsi'] ?>">
    </div>
    <div class="form-group">
        <label>Meta Keywords</label>
        <input type="text" class="form-control" name="seo_keywords" value="<?php echo $item['meta_keyword'] ?>">
    </div>
    </div>
...
<div class="box-footer text-right">
    <button class="btn btn-success" type="submit" name="additems" value="true">Update Product(s)</button>
</div>
</form>
</section>
...
function disselect(){
    $("body .select2.color-select").each(function(){
      ...
      }).prop('disabled', true).siblings().prop('disabled', false);
    });
}
...
function loadtype(type, newname){
    if(type == 1){
...
<?php if($item['ItemType'] == 3){ ?>
$("body").find('.disselectSize.type3_rec').closest('.size_product').find(".select2.disselectSize").each(function(){
      var val = $(this).select2({
        minimumResultsForSearch: -1
}).find(":selected").val();
      $("body").find('.disselectSize.type3_rec').closest('.size_product').find('.select2.disselectSize').not(this).find('option').filter(function() {
      return this.value === val;
      }).prop('disabled', true).siblings().prop('disabled', false);
});
<?php } ?>
