...
<div class="box">
  ...
  <div class="box-tools">
   <button class="btn btn-success pull-right" data-toggle="modal" data-target="#myModal"><i class="fa fa-plus"></i> Tambah Banner</button>     
  </div>
  ...
  <table id="table-01" class="table table-bordered">
    ...
    <td><?php echo ($key->type == '2') ? 'Video' : 'Image'; ?></td>
    <td><?php echo ($key->side == '2') ? 'Right' : 'Left'; ?></td>
...
$('#myModal').on('hide.bs.modal', function (e) {
  ...
  $('#myForm').attr('action', '<?php echo base_url().'process_entersite/banner' ?>');
...
$(".update_banner_modal").on('click', function(){
  $('#myModal').modal('show');
    ...
    $('#myForm').attr('action', '<?php echo base_url().'process_entersite/banner_edit/' ?>'+id);
