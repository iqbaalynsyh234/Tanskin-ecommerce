...
<form id="private_form" action="" method="post">
		...
		<div class="box">
		  <div class="box-body">
		  	<div class="row row-mar">
			    ...
			    <h2 id="total_amount" class="box-title" data-total="<?php echo $this->cart->total() ?>" style="margin: 10px 0px;"><b class="priceformat"><?php echo rupiah($this->cart->total()) ?></b></h2>
			    </div>
		  	</div>
		  </div>
		</div>
    ...
    $.ajax({
        ...
        success: function(json){
            if(json.status == '1'){
              ...
              $('.showdistrict, .showsubdistrict, .shipping_option').html('');
              $('.showdistrict, .showsubdistrict, .shipping_option').prop('disabled', true);
            }
            if(json.status == '0'){
              $('.showcity, .showdistrict, .showsubdistrict, .shipping_option').html('');
              $('.showcity, .showdistrict, .showsubdistrict, .shipping_option').prop('disabled', true);
            }
        }
    });
}
    ...
    $.ajax({
        ...
        success: function(json){
            if(json.status == '1'){
              ...
              $('.showsubdistrict, .shipping_option').html('');
              $('.showsubdistrict, .shipping_option').prop('disabled', true);
            }
            if(json.status == '0'){
              $('.showdistrict, .showsubdistrict, .shipping_option').html('');
              $('.showdistrict, .showsubdistrict, .shipping_option').prop('disabled', true);
...
$(function(){
	...
	$("#addtocart").validate({
		...
		var area_pengiriman = $(".showsubdistrict").val();
		$.ajax({
	        url: form.action,
	        type: form.method,
	        cache: false,
	        ...
	        success: function(json){
	            if(json.status == '0'){
	            	...
	            	$("#total_amount").data('total', json.total);
	            	init_getshipping_cost(area_pengiriman);
	            	init_total(0);
	            }
	        }
	    });
	...
	$(".shipping_option").on("change", function(){
		var total_2 = $(".shipping_option").find(':selected').data('cost');
		$('input[name="cost"]').val(total_2);
		init_total(total_2);
	});
});
</script>